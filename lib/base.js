...
var $ = module.exports = {
	...
	placeholder:{
		csscomboLink : function(url){
			 // return '<link type="text/css" rel="stylesheet"  href="'+url+'?v='+$.getTimestamp()+'" source="combo">\r\n';
		...
		cssLinkInline : function(url){
			 return '<link inline rel="stylesheet"  href="'+url+'">\r\n';
		},
		jsLinkInline : function(url){
			 return '<script inline src="'+url+'"></script>\r\n';
		},
		cssHtmlLink:function(cdn){
			return '<link rel="stylesheet" type="text/css" href="'+cdn+'">\r\n';
		},
		jsHtmlLink:function(cdn){
			return '<script src="'+cdn+'"></script>\r\n';
		},
		insertHead:function(content,str){
			if ( /<\/head>/.test(content) ){
				return content.replace('</head>',str+'\r\n</head>');
			}else{
				return str + content;
			}
		},
	...
	is:{
		...
		ftlSuffix: '.ftl',
		ftl : function(pathName){
			 return path.extname(pathName) === this.ftlSuffix;
		},
		vmSuffix: '.vm',
