	...
	if(source){
		if(f.isDir(source)){
			...
			// if( $.is.less(source)) {
				...
				if($.is.less(source) && gfe.config.build.less){
					try{
						Less.render(sourceContent, {filename: source, syncImport:true}, function(error, output){
							if(error){
								...
								if(gfe.config.build.csslint) fileLint.init(source);
								f.write(target, targetContent);
							}
						});
					}catch(e){
						console.log('gfe error [gfe.buildCss] - less\r\n'+source);
						console.log(e);
					}
				}

				if($.is.sass(source) && gfe.config.build.sass){
					try{
						...
						console.log('gfe error [gfe.buildCss] - sass\r\n'+source);
						console.log(e);
					}
				}
			}
