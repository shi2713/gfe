	...
	function route(file){
		...
		if(exists){
			if($.is.html(file) || $.is.vm(file) || $.is.tpl(file)){
				...
				console.log('gfe warning: can not lint the [' + file + '].\n');
			}
		}else{
			console.log('gfe error: the [' + file + '] may be not exist.');
		}
	}
}
...
function htmlLint(filename){
	...
	console.log(colors.yellow(os.EOL + 'gfe htmllint: ', filename));
...
function cssLint(filename){
	...
	if(result && result.messages.length){
		...
		console.log(colors.yellow(os.EOL + 'gfe csslint: ' + filename));
		...
		console.log('gfe csslint: ' + filename + ' is ok' );
	}
}
...
function jsLint(filename){
    ...
    if(result) {
        ...
        console.log(colors.yellow(os.EOL + 'gfe jslint: ' + filename));
...
function httpLinkLint(link){
	$.httpget(link, function(data){
		console.log(colors.yellow(os.EOL + 'gfe http-link-lint: ', link));
