...
*/
var options = {};
options.port = gfe.config.port || 21;
options.host = gfe.config.host || null;
options.user = gfe.config.user || null;
options.password = gfe.config.password || null;
...
ftp.upload = function(source, target, include, exclude, uncover, move, logTag, callback) {
    ...
    ftp.tryConnect().done(function() {
        ...
        console.log('gfe error [ftp.upload] - ' + err);
...
ftp.put = function(source, target, callback) {
    ftp.client.put(source, target, function(err) {
        if (err) {
            console.log('gfe error [ftp.put] - ' + target + ' - ' + err);
...
ftp.get = function(source, target, callback) {
    ftp.client.get(source, function(err, stream) {
        if (err) {
            console.log('gfe error [ftp.get] - ' + target + ' - ' + err);
            if (callback) callback(err);
            ftp.client.end();
        } else {
...
ftp.listMain = function(source, callback) {
    ftp.client.list(source, function(err, list) {
        if (err) {
            console.log('gfe error [ftp.list] - ' + err);
            ftp.client.end();
        } else {
            ...
            if (list && list.length > 0) {
                ...
                if (callback) {
                    ...
                    console.log('gfe warnning : "' + source + '" is not exists');
                };
                //ftp.client.end();
...
ftp.mkdir = function(source, callback) {
    if (typeof(source) == 'undefined') {
        console.log('gfe error [ftp.mkdir] source is not exists');
        return;
    }
